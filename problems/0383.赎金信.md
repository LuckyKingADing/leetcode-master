        ...
        for (int i = 0; i < magazine.length(); i++) {
            for (int j = 0; j < ransomNote.length(); j++) {
                ...
                if (magazine[i] == ransomNote[j]) {
                    // ransomNote.begin() + j：迭代器算术运算。begin() 指向索引 0，+ j 移动到索引 j 的位置（0-based）。
...
因为题目说只有小写字母，那可以采用**空间换取时间的哈希策略***，用一个长度为26的数组来记录magazine里字母出现的次数。
...
public:
    bool canConstruct(string ransomNote, string magazine) {
        ...
        for (int j = 0; j < ransomNote.length(); j++) {
            ...
            // 如果小于零说明ransomNote里出现的字符，magazine没有；等于0没事，小于0就有事了
            if(record[ransomNote[j]-'a'] < 0) {
                return false;
            }
        }
        // 说明没有进入false小于0的逻辑，就是true的
        return true;
    }
};
```
