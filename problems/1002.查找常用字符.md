...
for (int i = 0; i < A[0].size(); i++) { // 用第一个字符串给hash初始化;A[0]表示A中的第一个字符串，因为A是由若干个字符串组成的
    hash[A[0][i] - 'a']++; // 的一个字符串中包含的字母相对于字母a的距离，0表示a，1表示b，'a'就是ASCIi值
}
```
...
for (int i = 1; i < A.size(); i++) {
    memset(hashOtherStr, 0, 26 * sizeof(int)); // 清空临时数组
    for (int j = 0; j < A[i].size(); j++) { // 双层循环，遍历除第一个之外的其他字符串数组
        hashOtherStr[A[i][j] - 'a']++;
    }
    // 这是关键所在，关键：26个字母逐个遍历，取出数量的最小值，也有可能每个字符串中都含有多个相同字母，这就需要保留，用于后续输出，因此取最小值
...
for (int i = 0; i < 26; i++) {
    while (hash[i] != 0) { // 注意这里是while，多个重复的字符
        ...
        result.push_back(s);
        hash[i]--; // 数量-1,因此是while，要取出所有重复的字符
    }
}
```
